...
class WindowsTargetWindow:
    ...
    def display_name(self) -> str:
        executable = self.executable or "unknown process"
        return f"{self.title} ({executable}, PID {self.process_id})"


class WindowsBackgroundInput:
    """Window-aware Windows input that preserves the user's foreground app.

    Foreground targets use native SendInput for compatibility. Background targets
    receive direct Win32 window messages, so automation does not steal focus.
    The target is discovered by both window title and owning process.
    """
    ...
    def find_window(self) -> WindowsTargetWindow:
        pattern = (self.window_pattern or "Roblox").lower()
        candidates = self.list_windows()

        matching_process_and_title = []
        matching_process = []
        exact_title = []
        partial_title = []
        ...
        for window in candidates:
            ...
            if process_match and pattern in title:
                matching_process_and_title.append(window)
            if process_match:
        ...
        if matching_process_and_title:
            return matching_process_and_title[0]
        if matching_process:
    ...
    def _send_input_key(vk: int, down: bool) -> None:
        ...
        WindowsInputBackend()._send_key(vk, down)
